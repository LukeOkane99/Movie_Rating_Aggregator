...
def test_invalid_route(test_client, init_database):
    """
    GIVEN a flask application
    WHEN the '/fwgwgwg' page is requested (GET)
    THEN check the response is invalid (404)
    """
    response = test_client.get('/fwgwgwg')
    assert response.status_code == 404
    assert b'Oops.. This Page cannot be found!' in response.data
    assert b'<h1 style="text-align: center; font-family: sans-serif; color: whitesmoke; font-size: 200px;">404</h1>' in response.data