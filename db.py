...
def add_movie_to_db(ttl, yr, imdb, metacritic, tomatometer, audience_score, letterboxd, tmdb, average):
    with sqlite3.connect('database.db') as conn:
        ...
        cur.execute('SELECT rowid FROM Movies WHERE Title = (?) AND Year = (?)', (ttl,year))
        record = cur.fetchall()
        if (len(record) != 0):
...
#add_movie_to_db(title, year, imdb, metascore, tomatometer, audience_score, letterboxd, tmdb, avg)
print(get_movie(movie, release_year))